...
bool stream2tree(std::istream &is, TREE &tree, idx2name &names, WEIGHTS &weights) {
...
//             else ERROR_return("cannot read weight/branch length value " << input.getLastPos());
            ...
            if (name.empty()) ERROR_return/*ERROR_exit*/("problem in the tree expression " << input.getLastPos());
            names.insert(idx2name::value_type(lin, name));
        }
        if (!input.nextAnyChar(c)) return false;
    }
    if (rooting.compare("[&U]")==0) tree.unroot();
    else
    if (tree.empty()) tree.unroot();
    else tree.root = default_root;
    return true;
}
template<class TREE>
inline bool stream2tree(std::istream &is, TREE &tree, idx2name &names) {
    idx2weight weights;
    return stream2tree(is, tree, names, weights);
}
template<class TREE>
...
//             { // print weight
//                 idx2weight::iterator itr = weights.find(v.idx);
//                 if (itr != weights.end()) os << ':' << fixed << setprecision(20) << itr->second;
//             }
