...
int main(int argc, char* argv[]) { 
  ...
  /* Variables for leafcount and node count */
  int leafn = 0;
  ...
  */
  ...
  int initial_leaf_lca_array_index[MAXADDLEAF];
  int initial_lca_array[MAXADDLEAF];
  ...
  if(ifs2.is_open()){
    ...
    while(ifs2.good()) {  
      ...
      initial_leaf_lca_array_index[leafcount++] = lca_id;
      
      /* Store the value of lca in the lca array at specified index */
      initial_lca_array[lca_id] = lca_id; //This will be modified later if the new leaf is added to the stem of this family
