...
int main(int argc, char* argv[]) { 
  ...
  if(argc!=4){
    cout<<"Incorrect Arguments ! Exiting! \n\t usage - ./exec tree_file leaves_file opfile\n";
    exit(1);
  }
  ...
  for(int j=0; j<leafcount; j++) {
    ...
    unsigned int untranslated_node = binarysearch( bl_array, currnodecount-1, randomblength); 
    ...
    //cout<<"\nAdded Number of edges is "<<edgec;
    //cout<<"\nAdded Number of nodes is "<<nodec;
